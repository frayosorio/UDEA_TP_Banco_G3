...
public class FrmBanco extends JFrame {
    ...
    private void btnQuitarCuentaClick() {
        if (tblCuentas.getSelectedRow() >= 0) {
            CuentaServicio.quitar(tblCuentas.getSelectedRow());
            cmbCuenta.removeItemAt(tblCuentas.getSelectedRow());
            CuentaServicio.mostrar(tblCuentas);
        } else {
    ...
    private void btnGuardarCuentaClick() {
        ...
        try {
            ...
            if (!titular.isEmpty() &&
                    !numero.isEmpty() &&
                    saldoInicial > 0) {
                Cuenta cuenta = CuentaServicio.agregar(titular,
                        numero,
                        (TipoCuenta) cmbTipoCuenta.getSelectedItem(),
                        saldoInicial,
                        sobregiro,
                        plazo);
    ...
    private void btnGuardarTransaccionClick() {
        ...
        try {
            ...
            if (cmbCuenta.getSelectedIndex() >= 0 &&
                    cmbTipoTransaccion.getSelectedIndex() >= 0 &&
                    valor > 0) {
                ...
                JOptionPane.showMessageDialog(null,
                        "Debe seleccionar la cuenta y el tipo de transacción, asi como especificar un valor positivo de la transacción");
            }
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(this, ex.getMessage());
        }
